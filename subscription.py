...
from urllib.parse import unquote, parse_qs, urlparse, unquote_plus
...
def _decode_profile_title(headers: dict) -> str | None:
    """نام واقعی سرویس را از هدرهای نام‌گذاری رایج Subscription پیدا می‌کند.

    بعضی پنل‌ها ``Profile-Title`` را عمداً روی مقدار عمومی ``Subscription``
    می‌گذارند؛ بنابراین چند هدر رایج را امتحان می‌کنیم و Base64/Base64URL/URL
    encoding را هم پشتیبانی می‌کنیم.
    """
    generic = {"subscription", "profile", "sub", "default", "config", "configs"}
    header_names = (
        "Profile-Title", "profile-title", "X-Profile-Title", "x-profile-title",
        "Subscription-Title", "subscription-title", "X-Subscription-Name",
        "x-subscription-name", "Content-Disposition", "content-disposition",
    )
    for header_name in header_names:
        ...
        # Content-Disposition: attachment; filename="Config_430485"
        m = re.search(r'filename\*?=(?:UTF-8\'\')?"?([^";]+)', raw, re.I)
        if m:
            raw = unquote(m.group(1)).strip()
        raw = re.sub(r"^base64:", "", raw, flags=re.I).strip()
        candidates = [raw]
        try:
            decoded = base64.urlsafe_b64decode(raw + "=" * (-len(raw) % 4)).decode("utf-8", errors="ignore").strip()
            if decoded:
                candidates.insert(0, decoded)
        except Exception:
            pass
        for candidate in candidates:
            candidate = unquote_plus(candidate).strip().strip('"')
            if candidate and candidate.casefold() not in generic and len(candidate) < 200:
                return candidate
    return None
...
def _extract_name_from_body(body: str) -> str | None:
    """نام واقعی سرویس را از بدنه‌ی Subscription استخراج می‌کند.

    علاوه بر remarkهای VLESS/VMess، پاسخ‌های JSON/YAML و چندلایه Base64 هم
    بررسی می‌شوند. این مهم است چون بعضی پنل‌ها ``Profile-Title: Subscription``
    می‌فرستند ولی نام واقعی مثل ``Config_430485`` فقط در خود payload وجود دارد.
    """
    if not body:
        return None
    generic = {"subscription", "profile", "sub", "default", "config", "configs"}
    ...
    for _ in range(3):
        try:
            decoded = base64.urlsafe_b64decode(current + "=" * (-len(current) % 4)).decode("utf-8", errors="ignore").strip()
        except Exception:
            break
    ...
    def usable(value):
        if not isinstance(value, str):
            return None
        value = unquote_plus(value).strip().strip('"\'')
        if not value or value.casefold() in generic or len(value) > 200:
            return None
        return value

    def walk_json(value):
        if isinstance(value, dict):
            # این کلیدها بیشترین احتمال را برای نام پروفایل دارند.
            for key in ("profile-title", "profile_title", "profileTitle", "name", "remark", "remarks", "ps", "title"):
                if key in value:
                    found = usable(value.get(key))
                    if found:
                        return found
            for child in value.values():
                found = walk_json(child)
                if found:
                    return found
        ...
        for line in lines:
            ...
            if "://" in line:
                try:
                    ...
                    for key in ("name", "remark", "remarks", "profile-title", "profile_title", "profile"):
                        vals = qs.get(key)
                        if vals:
                            found = usable(vals[0])
                            if found:
            ...
            if line.lower().startswith("vmess://"):
                try:
                    ...
                    raw = base64.urlsafe_b64decode(payload + "=" * (-len(payload) % 4)).decode("utf-8", errors="ignore")
                    found = walk_json(json.loads(raw))
                    if found:
                        return found
                except Exception:
                    pass

            # YAML/متن ساده: name: Config_430485 / remark: Config_430485
            m = re.match(r"^(?:name|remark|remarks|profile[-_ ]?title)\s*:\s*[\"']?(.+?)[\"']?\s*$", line, re.I)
            if m:
                found = usable(m.group(1))
                if found:
                    return found
