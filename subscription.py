...
async def _get_panel_display_name(cfg: dict) -> str | None:
    ...
    try:
        ...
        elif source == "pasargad":
            import pasargad
            ok, data, _ = await pasargad.get_user(service_id)
        else:
...
async def enrich_configs_with_subscription_names(configs: list[dict]) -> list[dict]:
    """نام نمایشی را از پنل اصلی سرویس و در صورت نیاز از Subscription پر می‌کند.

    ترتیب منبع‌ها مهم است: اول نام واقعی همان سرویس از پنل خوانده می‌شود؛
    اگر پنل در دسترس نبود، از Profile-Title/remark داخل Subscription استفاده می‌کنیم.
    """
