...
async def profile(callback: types.CallbackQuery):
    ...
    if entities:
        entities = await _repair_custom_emoji_entities(callback.bot, str(text), entities)
    await show_menu_with_sticker(callback.bot, callback.message.chat.id, "profile", text, reply_markup=profile_menu(), entities=entities)
