...
async def menu_profile(message: types.Message, state: FSMContext):
    ...
    if entities:
        entities = await _repair_custom_emoji_entities(message.bot, str(text), entities)
    await show_menu_with_sticker(message.bot, message.chat.id, "profile", text, reply_markup=profile_menu(), entities=entities)
