...
async def admin_config_search_result(message: types.Message, state: FSMContext):
    ...
    for cfg in configs:
        ...
        if q in display_name.casefold() or q in plan_name.casefold() or q in str(cfg.get("service_id") or "").casefold():
            matches.append(cfg)
...
def _guide_detail_text_and_entities(guide: dict):
    ...
    # Title entities are stored directly from Telegram so every Premium/Custom
    # Emoji (not only the first one) survives in both admin and user views.
    title_entities = message_entities_from_dicts(guide.get("title_entities") or [])
    for entity in title_entities:
        try:
            entities.append(entity.model_copy(update={"offset": int(entity.offset) + telegram_utf16_length("📚 ")}))
        except Exception:
            pass

    # Backward compatibility for old guides created before title_entities was persisted.
        ...
        try:
            ...
            if emoji_id:
                ...
                while j < len(title):
                    ...
                    break
                if span:
                    entities.append(types.MessageEntity(type="custom_emoji", offset=telegram_utf16_length("📚 "), length=span, custom_emoji_id=str(emoji_id)))
        except Exception:
...
async def _send_guide_detail(target: types.Message, guide_id: int):
    ...
    try:
        entities = await _repair_custom_emoji_entities(target.bot, caption, entities) if entities else []
