...
async def show_menu_with_sticker(
    ...
    # متن‌های قابل‌ویرایش Entityهای واقعی Telegram را با offset نهایی خودشان حمل
    # می‌کنند. اینجا فقط اعتبارسنجی می‌کنیم؛ repair کردن دوباره می‌تواند Custom
    # Emojiهای مشابه را روی هم جابه‌جا کند و باعث شود فقط بعضی Premium Emojiها
    # (مثل صفحه پروفایل) نمایش داده شوند.
    if entities:
        entities = _sanitize_entities_for_text(str(text), entities)
        parse_mode = None
