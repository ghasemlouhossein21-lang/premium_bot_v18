...
# ---------------------------------------------------------------------------
def _guide_title_custom_emoji_id(guide: dict) -> str | None:
    """اولین Premium Emoji عنوان راهنما را برای آیکن خود دکمه برمی‌گرداند."""
    for ent in guide.get("title_entities") or []:
        try:
            typ = ent.get("type")
            if hasattr(typ, "value"):
                typ = typ.value
            if typ == "custom_emoji" and ent.get("custom_emoji_id"):
                return str(ent["custom_emoji_id"])
        except Exception:
            continue
    # راهنماهای قدیمی که title_entities ندارند، از mapping قدیمی متن استفاده کنند.
    try:
        return db.get_button_custom_emoji_id("text:" + str(guide.get("title") or ""))
    except Exception:
        return None
...
def user_guides_menu(guides: list):
    if not guides:
        ...
        for g in guides:
            kwargs = {
                "text": str(g.get("title") or ""),
                "callback_data": f"guideopen_{g['id']}",
                "style": "primary",
            }
            emoji_id = _guide_title_custom_emoji_id(g)
            if emoji_id:
                kwargs["icon_custom_emoji_id"] = emoji_id
